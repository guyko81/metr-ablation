...
We also report $p_{80}$ threshold crossings---the task duration at which models achieve 80\% success---as an additional reference point for ``easy task'' capability.
...
The $G[T]$ metric yields a doubling-time estimate of $128$--$130$ days with $R^2 \geq 0.916$ across both methods, compared to $121$--$130$ days with $R^2$ ranging from $0.852$ to $0.916$ under $p_{50}$.
The $p_{80}$ threshold (easy-task frontier) shows doubling times of $111$--$121$ days with $R^2 = 0.881$--$0.901$.
...
For isotonic regression, we use the bootstrap median curve for threshold crossings rather than the single full-data fit, yielding smoother per-model values.
...
Indeed, switching from logistic to isotonic regression changes the per-model $p_{50}$ values dramatically even though the overall doubling time remains similar.
To mitigate this instability for isotonic regression, we report threshold crossings from the bootstrap \emph{median} curve rather than the single full-data fit, which produces smoother per-model values.
...
\begin{itemize}
  ...
  \item $p_{50}$ doubling time: $121$--$130$ days ($R^2 = 0.852$--$0.916$).
  \item $p_{80}$ doubling time: $111$--$121$ days ($R^2 = 0.881$--$0.901$).
  \item The $R^2$ spread across methods shrinks from $0.064$ ($p_{50}$) to $0.005$ ($G[T]$).
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Comparison of curve-fitting methods across all 18 models.
...
$p_{80}$ captures the ``easy task'' frontier where models achieve 80\% success.}
\label{tab:results}
\begin{tabular}{@{}lcccccccc@{}}
\toprule
\textbf{Method} & \textbf{CV-Brier} & \textbf{CV-LogLoss} & \textbf{$p_{50}$ Dbl} & \textbf{$p_{50}$ $R^2$} & \textbf{$p_{80}$ Dbl} & \textbf{$p_{80}$ $R^2$} & \textbf{$G[T]$ Dbl} & \textbf{$G[T]$ $R^2$} \\
\midrule
...
\begin{enumerate}
  ...
  \item \textbf{Metric stability:} $G[T]$ produces nearly identical doubling times ($128$--$130$\,d) and $R^2$ values ($0.916$--$0.921$) across both methods.
        Under $p_{50}$, the $R^2$ drops to $0.852$ for isotonic, and individual-model $p_{50}$ values change between methods even though the trend slope is preserved.
  \item \textbf{$p_{80}$ frontier:} The $p_{80}$ threshold captures how far models can go on ``easy'' tasks (80\% success rate). Its doubling time provides an alternative view of capability growth that is less sensitive to the transition zone around $p = 0.5$.
  \item \textbf{Headline robustness:} The doubling time is approximately four months ($\sim$\!$128$--$130$ days under $G[T]$) regardless of methodology.
...
\begin{itemize}
  ...
  \item Compare $p_{50}$, $p_{80}$, and $G[T]$ trend charts with regression lines and confidence intervals.
...
What changes is the \emph{defensibility} of the estimate---a $G[T]$-based trend with $R^2 = 0.921$ is harder to dismiss than a $p_{50}$-based trend with $R^2 = 0.852$.
