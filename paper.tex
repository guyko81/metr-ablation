...
We propose three concrete, drop-in improvements to METR's curve-fitting pipeline: (1)~replacing logistic regression with smoothed isotonic regression to avoid the symmetric-sigmoid assumption, (2)~replacing the $p_{50}$ threshold-crossing summary metric with a geometric-mean integral metric $G[T]$ that uses the entire fitted curve, and (3)~using $m$-out-of-$n$ bootstrap instead of standard bootstrap for theoretical consistency with shape-constrained estimators.
We validate each improvement with 5-fold cross-validation on 18 frontier AI models.
Smoothed isotonic regression improves cross-validated log-loss from $0.376$ to $0.363$; isotonic regression improves cross-validated Brier score from $0.1212$ to $0.1115$.
The $G[T]$ metric yields a doubling-time estimate of $129$--$130$ days with $R^2 \geq 0.921$ across all three methods, compared to $118$--$121$ days with $R^2$ ranging from $0.811$ to $0.916$ under $p_{50}$.
...
\url{https://[USERNAME].github.io/metr-ablation/} \hfill {\small(GitHub Pages)}

\medskip\noindent
\textbf{Reproducibility:} Full code and data at \url{https://github.com/[USERNAME]/metr-ablation}.
\end{abstract}
...
The doubling time moves from $121$ days (logistic + $p_{50}$) to $130$ days (smoothed isotonic + $G[T]$)---still approximately four months---but the estimate becomes more defensible against methodological critique.

Our three improvements are:
\begin{enumerate}
  \item \textbf{Curve fitting:} Replace logistic regression with smoothed isotonic regression to avoid the symmetric-sigmoid assumption.
...
All code, data, and interactive visualizations are publicly available.\footnote{Interactive viewer: \url{https://[USERNAME].github.io/metr-ablation/}; repository: \url{https://github.com/[USERNAME]/metr-ablation}}
...
Raw isotonic regression produces a step function, which is discontinuous and can cause threshold-crossing metrics to jump between steps.
\emph{Smoothed isotonic regression} addresses this by applying a Gaussian kernel in $\log_2$-space ($\sigma = 0.5$ log$_2$ units) after fitting, producing a smooth, monotone curve.
...
\begin{itemize}
  ...
  \item \textbf{Log-loss:} Smoothed isotonic achieves $0.363$ vs.\ logistic's $0.376$ (3.5\% improvement).
...
\paragraph{Evidence.}
\begin{itemize}
  \item $G[T]$ doubling time: $128$--$130$ days across all three methods ($R^2 = 0.916$--$0.921$).
...
In our implementation, we use $m$-out-of-$n$ bootstrap for both isotonic and smoothed isotonic methods, and standard bootstrap for logistic regression.
...
\begin{enumerate}
  \item \textbf{Fit quality:} Isotonic wins on CV-Brier; smoothed isotonic wins on CV-LogLoss. Both outperform logistic out of sample.
  \item \textbf{Metric stability:} $G[T]$ produces nearly identical doubling times ($128$--$130$\,d) and $R^2$ values ($0.916$--$0.921$) across all methods.
        Under $p_{50}$, the $R^2$ drops to $0.811$ for isotonic due to the step-function artifact.
  \item \textbf{Headline robustness:} The doubling time is approximately four months ($\sim$\!$128$--$130$ days) regardless of methodology.
  \item \textbf{Recommendation:} Use smoothed isotonic regression with $G[T]$ for the most defensible estimate. This combination achieves the best cross-validated log-loss and the most stable trend fit.
\end{enumerate}
...
\begin{center}
  \url{https://[USERNAME].github.io/metr-ablation/}
\end{center}
The viewer allows readers to:
\begin{itemize}
  \item Switch between all three fitting methods and compare aggregate statistics side by side.
...
The doubling time of AI agent capabilities is approximately four months, and this holds across logistic, isotonic, and smoothed isotonic curve fitting, and across both $p_{50}$ and $G[T]$ summary metrics.
...
The recommended methodology (smoothed isotonic + $G[T]$) provides a principled, well-calibrated, and stable estimate that should withstand methodological scrutiny.
